type ores = [number, number, number, number, number]  // 0 - железо, 1 - медь, 2 - олово, 3 - золото, 4 - драгоценные камни
...
class Miner {
    ...
    constructor(x: number, y: number, type: MinerType) {
        this.x = x
        this.y = y
        this.type = type
    }

    getMiningResults(): ores {
...
class MinerType {
    ...
    constructor(typeName: string, miningStats: ores) {
...
class MinerFactory {
    types: Map<string, MinerType>

    constructor() {
        this.types = new Map<string, MinerType>()
    }

    getMinerType(typeName: string, miningStats: ores): any {
    ...
    createUniversal1(x: number, y: number) {
        let typeName = "Universal miner LVL 1"
        let miningStats: ores = [2, 2, 1, 0, 0]
        return new Miner(x, y, this.getMinerType(typeName, miningStats))
    }

    createUniversal2(x: number, y: number) {
        let typeName = "Universal miner LVL 2"
        let miningStats: ores = [6, 5, 5, 1, 1]
        return new Miner(x, y, this.getMinerType(typeName, miningStats))
    }

    createGem1(x: number, y: number) {
        let typeName = "Gem miner LVL 1"
        let miningStats: ores = [0, 0, 0, 0, 3]
        return new Miner(x, y, this.getMinerType(typeName, miningStats))
    }

    createGold1(x: number, y: number) {
        let typeName = "Gold miner LVL 1"
        let miningStats: ores = [0, 0, 0, 7, 0]
        return new Miner(x, y, this.getMinerType(typeName, miningStats))
    }
}
...
class Mine {
    private factory: MinerFactory
    private miners: Miner[]
    private totalResourcesMined: ores
...
Драгоценные камни: ${this.totalResourcesMined[4]}`
    ...
    addUniversal1(x: number = 0, y: number = 0): void {
        this.miners.push(this.factory.createUniversal1(x, y))
    }

    addUniversal2(x: number = 0, y: number = 0): void {
        this.miners.push(this.factory.createUniversal2(x, y))
    }

    addGem1(x: number = 0, y: number = 0): void {
        this.miners.push(this.factory.createGem1(x, y))
    }

    addGold1(x: number = 0, y: number = 0): void {
